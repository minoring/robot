...
// Define global vector of joints last position, moving state of arm, and the client that ca request services
...
// This callbacak function continuously executes and reads the arm joint angles position
...
// This callback funciton continuously executes and reads the image data
void look_away_callback(const sensor_msgs::Image img) {
  bool uniform_image = true;

  // Loop through each pixel in the image and check if it is equal to the first cone.
  ...
  // If the image is uniform and the arm is not moving, move the arm to the cenver
...
int main(int argc, char** argv) {
  ...
  // Subscribe to /simple_arm/joint_states topic to read the arm 
  // joints position inside the joint_states_callback function
